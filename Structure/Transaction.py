...
def get_timestamp(block_time,UTC=True):
	...
	elif isinstance(block_time,datetime.datetime):
		if UTC:
			dt = block_time + datetime.timedelta(hours=8)
		else:
			dt = block_time
	else:
...
class Transaction:
	def __init__(self,tx_hash:str,msg_caller:str,msg_to:str,msg_value:int,msg_input:str,timestamp):
